...
Module.register("MMM-network-signal", {
	...
	defaults: {
		...
		flexDirection: 'row', // set to 'row' to start the row from the left, 'row-reverse' to start the row from the right
	...
	getDom: function() {
		...
		switch (true) {
			...
			case this.ping < this.config.thresholds.strong:
				wifiSign.src = this.file("icons/strong.png");
				if (this.config.showMessage) {
			...
			case this.ping < this.config.thresholds.medium:
				wifiSign.src = this.file("icons/medium.png");
				if (this.config.showMessage) {
			...
			case this.ping < this.config.thresholds.weak:
				wifiSign.src = this.file("icons/weak.png");
				if (this.config.showMessage) {
			...
			case this.ping > this.config.thresholds.weak:
				wifiSign.src = this.file("icons/no_signal.png");
				if (this.config.showMessage) {
